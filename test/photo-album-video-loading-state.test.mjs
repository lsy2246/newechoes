...
test("PhotoAlbumMasonry surfaces preview loading progress and buffering state", () => {
  ...
  assert.match(component, /const isPreviewVideoSpinnerVisible =\s*previewVideoRequested && \(!previewVideoReady \|\| previewVideoBuffering\);/);
  ...
  assert.match(component, /onCanPlay=\{\(event\) => \{[\s\S]*setPreviewVideoBuffering\(false\);[\s\S]*syncPreviewVideoLoadProgress\(event\.currentTarget\);/);
  assert.match(component, /onPlaying=\{\(\) => \{[\s\S]*setPreviewVideoStarted\(true\);[\s\S]*setPreviewVideoBuffering\(false\);/);
