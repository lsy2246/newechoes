...
test("server asset relay encodes source request headers into the relay URL", () => {
  ...
  assert.doesNotMatch(serverRelaySource, /cacheTtl|cache_ttl|cacheKeyMode|cache_key_mode|cacheKey|cache_key/);
  assert.doesNotMatch(serverRelaySource, /searchParams\.set\("cache"/);
  assert.doesNotMatch(serverRelaySource, /mode\?:|searchParams\.set\("mode"/);
...
test("Douban prefers relay URLs in the frontend and falls back to the local image API", () => {
  ...
  assert.doesNotMatch(doubanRouteSource, /cache:\s*"prefer"|cacheTtl|DOUBAN_IMAGE_CACHE_TTL_SECONDS/);
...
test("Google Photos page/media flow keeps relay-first images and standard proxy videos with backend fallback", () => {
  assert.match(googlePhotosSource, /server\/asset-relay/);
  assert.match(googlePhotosSource, /GOOGLE_PHOTOS_MEDIA_HEADERS/);
  assert.doesNotMatch(googlePhotosSource, /GOOGLE_PHOTOS_PAGE_CACHE_TTL_SECONDS|GOOGLE_PHOTOS_MEDIA_CACHE_TTL_SECONDS/);
  assert.doesNotMatch(googlePhotosSource, /cache:\s*"prefer"|cacheTtl|mode:\s*"media"/);
  ...
  assert.match(googlePhotosSource, /relayAssetUrl\(\s*mediaUrl,\s*GOOGLE_PHOTOS_MEDIA_HEADERS\s*\)\s*\|\|\s*fallbackUrl/);
  assert.match(googlePhotosSource, /const streamedGooglePhotosVideoUrl = \(baseUrl: string\) => \{/);
  assert.match(googlePhotosSource, /const relayUrl = relayAssetUrl\(\s*mediaUrl,\s*GOOGLE_PHOTOS_MEDIA_HEADERS\s*\);/);
  ...
  assert.match(googlePhotosSource, /fetchAssetWithRelayFallback\(\s*shareUrl,\s*\{[\s\S]*headers:\s*GOOGLE_PHOTOS_PAGE_HEADERS[\s\S]*signal:\s*controller\.signal/);
