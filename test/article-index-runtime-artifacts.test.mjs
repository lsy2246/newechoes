...
test("article index build writes a prebuilt article history artifact", () => {
  ...
  assert.match(buildHelperSource, /getArticleHistory/);
});

test("article index integration no longer loads source repository config at runtime", () => {
  assert.equal(existsSync("src/config/source-repository.js"), false);
  assert.doesNotMatch(buildPluginSource, /consts\.ts/);
});
