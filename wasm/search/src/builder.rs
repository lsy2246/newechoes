...
impl SearchBuilder {
    ...
    pub fn build_search_index(&self) -> Result<ArticleSearchIndex, String> {
        ...
        let stop_words: HashSet<&str> = [
            "的", "是", "在", "了", "和", "与", "或", "而", "但", "如果", "因为", "所以", "这",
            "那", "这个", "那个", "这些", "那些", "并", "可以", "把", "被", "将", "已", "就", "也",
            "很", "到", "上", "下", "中", "为",
        ]
        .iter()
        .cloned()
    ...
    pub fn save_search_index(&self, path: &str) -> Result<(), String> {
        ...
        // 保存搜索索引
        let mut search_file =
            File::create(path).map_err(|e| format!("无法创建搜索索引文件: {}", e))?;
    ...
    /// 构建标题关键词索引
    fn build_heading_term_index(
        &self,
        headings: &HashMap<String, HeadingIndexEntry>,
    ...
    fn build_title_term_index(&self) -> HashMap<String, HashSet<usize>> {
        ...
        for (article_id, article) in self.articles.iter().enumerate() {
            ...
            // 额外处理：标题中的各个单词
            let title_words: Vec<String> = article
                .title
                .to_lowercase()
                .split_whitespace()
