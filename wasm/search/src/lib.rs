use crate::models::{
    ArticleSearchIndex, HeadingIndexEntry, HeadingNode, SearchRequest, SearchResult,
    SearchResultItem, SearchSuggestion, SuggestionCandidate, SuggestionType,
};
use js_sys::Date;
...
static SEARCH_INDEX: OnceCell<Mutex<Option<ArticleSearchIndex>>> = OnceCell::new();
...
fn run_search(search_index: &ArticleSearchIndex, req: &SearchRequest) -> SearchResult {
...
/// 初始化搜索索引缓存
#[wasm_bindgen]
pub fn init_search_index(index_data: &[u8]) -> Result<(), JsValue> {
    set_panic_hook();
    ...
    let cell = SEARCH_INDEX.get_or_init(|| Mutex::new(None));
    let mut guard = cell
        .lock()
        .map_err(|_| JsValue::from_str("获取索引锁失败"))?;
...
/// 使用缓存索引执行搜索
#[wasm_bindgen]
pub fn search_cached(request_json: &str) -> Result<String, JsValue> {
    set_panic_hook();
    ...
    let cell = SEARCH_INDEX
        .get()
        .ok_or_else(|| JsValue::from_str("索引未初始化"))?;
    let guard = cell
        .lock()
        .map_err(|_| JsValue::from_str("获取索引锁失败"))?;
    ...
    let mut result = run_search(search_index, &req);
    let end_time = Date::now();
    result.time_ms = (end_time - start_time) as usize;
    ...
    if candidates.len() < 5 {
        for (term, freq) in &search_index.common_terms {
            ...
            if term_lower == query
                || candidates
                    .iter()
                    .any(|s| s.text.to_lowercase() == term_lower)
            {
                continue;
            }
        ...
        .map(|candidate| {
            ...
            let (matched_text, suggestion_text) = match candidate.suggestion_type {
                SuggestionType::Completion if text_lower.starts_with(&query) => {
                    ...
                    (
                        original_case_matched.to_string(),
                        original_case_suggestion.to_string(),
                    )
                }
                _ => {
...
fn highlight_title(title: &str, query: &str) -> String {
    ...
    while start_idx < title_lower.len() {
        if let Some(found_idx) = title_lower[start_idx..].find(&query_lower) {
            ...
            start_idx = if valid_match_end > start_idx {
                valid_match_end
            } else {
                start_idx + 1
            };
...
fn format_matched_content(content: &str, term_positions: &[(usize, usize, i32)]) -> String {
    ...
    if content.len() > 300 {
        // 查找第一个高优先级匹配（通常是完整查询）
        let primary_matches: Vec<&(usize, usize, i32)> = term_positions
            .iter()
        ...
        let mut ctx_start = if first_start > 150 {
            first_start - 150
        } else {
            0
        };
        ...
        let mut last_pos = 0;
        // 只处理在上下文范围内的匹配
        let visible_matches: Vec<(usize, usize)> = term_positions
            .iter()
...
fn build_heading_tree_with_matches(
    ...
    let heading_map: HashMap<String, &HeadingIndexEntry> = search_index
        .heading_index
        .iter()
    ...
    if heading_map.is_empty() {
        ...
        // 查找全文匹配
        if let Some((highlighted_content, matched_terms)) =
            find_matches_in_paragraph(article, &root_heading, terms)
        {
    ...
    // 查找根标题（没有父标题的标题）
    let mut root_headings: Vec<&&HeadingIndexEntry> = heading_map
        .values()
    ...
    for (heading_id, heading) in &heading_map {
        if let Some((highlighted_content, matched_terms)) =
            find_matches_in_paragraph(article, heading, terms)
        {
    ...
    let root_content = if let Some((highlighted_content, matched_terms)) =
        find_matches_in_paragraph(article, &root_heading, terms)
    {
        Some((highlighted_content, matched_terms))
    } else {
    ...
    root_node
        .children
        .sort_by(|a, b| match a.level.cmp(&b.level) {
...
fn process_deeper_nodes(
    ...
    // 按级别和文本排序子节点
    parent.children.sort_by(|a, b| match a.level.cmp(&b.level) {
        std::cmp::Ordering::Equal => a.text.cmp(&b.text),
        other => other,
    });
}
